...
<div class="flex items-center justify-between mb-4">
    <h1 class="text-2xl font-bold">Agama</h1>
    <a href="{{ route('admin.agama.create') }}"
       ...
       <span>＋</span><span>Tambah Agama</span>
    </a>
</div>
...
{{-- 🔍 Form Pencarian --}}
<form method="GET" action="{{ route('admin.agama.index') }}" class="mb-4 flex space-x-2">
        ...
        <thead class="bg-gray-900 text-white uppercase text-xs tracking-wider">
            <tr>
                ...
                <th class="px-6 py-3 text-left">
                    <a href="{{ route('admin.agama.index', ['sort' => 'nama', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc', 'q' => request('q')]) }}"
            ...
            @forelse ($items as $index => $i)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-center flex items-center justify-center space-x-2">
                        {{-- Tombol Edit --}}
                        <a href="{{ route('admin.agama.edit',$i) }}"
                        ...
                        {{-- Tombol Hapus --}}
                        <form method="POST" action="{{ route('admin.agama.destroy',$i) }}" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                onclick="return confirm('Hapus agama ini?')"
